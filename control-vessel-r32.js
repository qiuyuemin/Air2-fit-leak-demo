...
function c32Drops(flow,side){
  ...
  return count?`<span class="c32-drops c32-drops-${flow} c32-drops-${side}" data-flow-kind="${flow}">${Array.from({length:count},function(_,i){var lane=lanes[(i+offset)%lanes.length];return `<img src="./assets/figma-control-r15/milk-drop.svg" style="--drop:${i};--drop-x:${lane}%" alt="">`;}).join('')}</span>`:'';
}
