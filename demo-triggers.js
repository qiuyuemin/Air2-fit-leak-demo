(function () {
  if (window.Air2DemoTriggers && window.Air2DemoTriggers.version === 60) return;
  var CAP = 6.09;
  var BASE_FLOW = 0;
  ...
  var triggers = [
    ...
    ['letdown-start-l','Left let-down starts',true,'letdown'],
    ['letdown-end-l','Left let-down ends',false,'letdown'],
    ['letdown-start-r','Right let-down starts',true,'letdown'],
    ['letdown-end-r','Right let-down ends',false,'letdown'],
  ...
  function notify(s,k,t,persist,backdrop){ var warm=!!backdrop||k==='auto'||k==='ending'||k==='suggestion'||k==='critical-battery'||String(t||'').length>44,id=Date.now(); s.controlNotice={kind:k,text:t,id:id,startedAt:id,persistent:!!persist,backdrop:warm}; if(!persist){ setTimeout(function(){ var screen;if(!(s.controlNotice&&s.controlNotice.id===id&&s.controlNotice.kind===k))return;screen=document.querySelector('#demo .v4-control.has-c36-notice');if(screen){screen.classList.add('air2-notice-leaving');setTimeout(function(){if(s.controlNotice&&s.controlNotice.id===id){s.controlNotice=null;paint()}},1250)}else{s.controlNotice=null;paint()} },5000); } }
  ...
  function beginSession(s){ s.page='control'; s.modal=null; s.running=true; s.paused=false; s.timer=0; s.milkL=0; s.milkR=0; s.mode='stimulation'; s.letdownPhase='baseline'; s.letdownPhaseL='baseline'; s.letdownPhaseR='baseline'; s.letdownEventAtL=null; s.letdownEventAtR=null; s.air2BothLetdownConfirmed=false; s.flowRate=BASE_FLOW; s.flowRateL=BASE_FLOW; s.flowRateR=BASE_FLOW; s.flowKind='none'; s.flowKindL='none'; s.flowKindR='none'; s.air2LetdownEvents=[]; s.air2LastPhysicsAt=Date.now(); s.air2SessionEnded=false; s.controlNotice=null; if(window.air2DemoRun) window.air2DemoRun={prompted:false,autoMoved:false,finishing:false,lastMilkTick:0}; }
  ...
  function resetBasePlan(s){ s.mode='stimulation'; s.selectedProgram=null; s.rhythmIndex=0; s.rhythmSequence=['stimulation','expression','stimulation','expression']; s.letdownPhase='baseline'; s.letdownPhaseL='baseline'; s.letdownPhaseR='baseline'; s.letdownEventAt=null; s.letdownEventAtL=null; s.letdownEventAtR=null; s.air2BothLetdownConfirmed=false; s.letdownStableSince=null; s.noMilkSince=null; s.letdownSuggestionShown=false; s.manualEndSuggestionShown=false; s.flowRate=BASE_FLOW; s.flowRateL=BASE_FLOW; s.flowRateR=BASE_FLOW; s.flowKind='none'; s.flowKindL='none'; s.flowKindR='none'; }
  ...
  function physics(){ var s=st(), now=Date.now(), flowL, flowR, flow, elapsed, addL, addR, bothActive, bothEnding; if(!s||!s.running||s.paused) return; flowL=flowNow(s,'l'); flowR=flowNow(s,'r'); flow=Math.max(flowL,flowR); s.flowRateL=Math.round(flowL*1000)/1000; s.flowRateR=Math.round(flowR*1000)/1000; s.flowRate=Math.round(((flowL+flowR)/2)*1000)/1000; s.flowKindL=flowKind(flowL); s.flowKindR=flowKind(flowR); s.flowKind=flowKind(flow); elapsed=Math.max(.25,Math.min(2,(now-(s.air2LastPhysicsAt||now))/1000)); s.air2LastPhysicsAt=now; addL=(flowL/28.3495)*elapsed*DEMO_MILK_ACCEL; addR=(flowR/28.3495)*elapsed*DEMO_MILK_ACCEL; s.milkL=Math.min(CAP,Math.round(((Number(s.milkL)||0)+addL*.98)*1000)/1000); s.milkR=Math.min(CAP,Math.round(((Number(s.milkR)||0)+addR*1.02)*1000)/1000);
    if(s.letdownPhaseL==='rising'&&flowL>=.005*28.3495&&now-(s.letdownEventAtL||now)>=SWITCH_CONFIRM_MS)s.letdownPhaseL='active'; if(s.letdownPhaseR==='rising'&&flowR>=.005*28.3495&&now-(s.letdownEventAtR||now)>=SWITCH_CONFIRM_MS)s.letdownPhaseR='active'; bothActive=s.letdownPhaseL==='active'&&s.letdownPhaseR==='active'; if(bothActive&&!s.air2BothLetdownConfirmed){s.air2BothLetdownConfirmed=true;s.letdownPhase='active';if(s.mode==='stimulation'){if(s.selectedProgram){s.mode='expression';s.rhythmIndex=Math.max(1,Number(s.rhythmIndex)||1);notify(s,'auto','Let-down detected on both sides. Moving into the Expression phase.',false)}else if(s.auto){s.mode='expression';notify(s,'auto','Let-down detected on both sides. Switched to Expression mode.',false)}else if(!s.letdownSuggestionShown){s.letdownSuggestionShown=true;notify(s,'suggestion','Let-down detected on both sides. You can switch to Expression when ready.',false)}}}
  ...
  function trigger(id){ var s=st(), match, side, suffix, sideName; if(!s) return false; s.lastIgnoredTrigger=''; if(id==='fit-ok') return fitOk(); if(!isActive(s)) return ignored(s,id); match=/^letdown-(start|end)-([lr])$/.exec(id); if(match){side=match[2];suffix=side==='r'?'R':'L';sideName=side==='r'?'Right':'Left';s['letdownPhase'+suffix]=match[1]==='start'?'rising':'falling';s['letdownEventAt'+suffix]=Date.now();s.letdownPhase=match[1]==='start'?'rising':s.letdownPhase;s.noMilkSince=null;s.manualEndSuggestionShown=false;if(match[1]==='start'){s.air2BothLetdownConfirmed=false;notify(s,'side-letdown',sideName+' let-down detected.',false)}else notify(s,'side-letdown',sideName+' let-down has ended.',false);recordLetdown(s,side,match[1]);paint();return true;} if(id==='air-leak'){ s.paused=true; s.severeLeak=true; s.leakSide='r'; s.leakAdjusting=true; s.flowRate=0; s.flowKind='paused'; s.controlNotice={kind:'leak',phase:'warning',text:'Air leak detected',id:Date.now()}; paint(); return true; } if(id==='low-battery'){ s.batteryL=12; s.batteryR=10; notify(s,'low-battery','Battery is running low. You can finish this session, then charge Air 2 soon.',false); paint(); return true; } if(id==='critical-battery') return criticalBattery(); if(id==='minor-leak'){ s.microLeakDuringSession=true; return true; } return false; }
  ...
  function boot(){ install(); wrapLogged(); wrapView(); mount(); window.Air2DemoTriggers={version:60,trigger:trigger,list:function(){return triggers;},sync:sync,physics:physics}; }
