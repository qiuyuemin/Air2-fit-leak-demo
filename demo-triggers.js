(function () {
  if (window.Air2DemoTriggers && window.Air2DemoTriggers.version === 58) return;
  var CAP = 6.09;
  ...
  function beginSession(s){ s.page='control'; s.modal=null; s.running=true; s.paused=false; s.timer=0; s.milkL=0; s.milkR=0; s.mode='stimulation'; s.letdownPhase='baseline'; s.letdownPhaseL='baseline'; s.letdownPhaseR='baseline'; s.letdownEventAtL=null; s.letdownEventAtR=null; s.flowRate=BASE_FLOW; s.flowRateL=BASE_FLOW; s.flowRateR=BASE_FLOW; s.flowKind='low'; s.flowKindL='low'; s.flowKindR='low'; s.air2LetdownEvents=[]; s.air2LastPhysicsAt=Date.now(); s.air2SessionEnded=false; s.controlNotice=null; if(window.air2DemoRun) window.air2DemoRun={prompted:false,autoMoved:false,finishing:false,lastMilkTick:0}; }
  ...
  function resetBasePlan(s){ s.mode='stimulation'; s.selectedProgram=null; s.rhythmIndex=0; s.rhythmSequence=['stimulation','expression','stimulation','expression']; s.letdownPhase='baseline'; s.letdownPhaseL='baseline'; s.letdownPhaseR='baseline'; s.letdownEventAt=null; s.letdownEventAtL=null; s.letdownEventAtR=null; s.letdownStableSince=null; s.noMilkSince=null; s.letdownSuggestionShown=false; s.manualEndSuggestionShown=false; s.flowRate=BASE_FLOW; s.flowRateL=BASE_FLOW; s.flowRateR=BASE_FLOW; s.flowKind='low'; s.flowKindL='low'; s.flowKindR='low'; }
  ...
  function physics(){ var s=st(), now=Date.now(), flowL, flowR, flow, elapsed, addL, addR, rising, bothEnding; if(!s||!s.running||s.paused) return; flowL=flowNow(s,'l'); flowR=flowNow(s,'r'); flow=Math.max(flowL,flowR); s.flowRateL=Math.round(flowL*1000)/1000; s.flowRateR=Math.round(flowR*1000)/1000; s.flowRate=Math.round(((flowL+flowR)/2)*1000)/1000; s.flowKindL=flowKind(flowL); s.flowKindR=flowKind(flowR); s.flowKind=flowKind(flow); elapsed=Math.max(.25,Math.min(2,(now-(s.air2LastPhysicsAt||now))/1000)); s.air2LastPhysicsAt=now; addL=(flowL/28.3495)*elapsed*DEMO_MILK_ACCEL; addR=(flowR/28.3495)*elapsed*DEMO_MILK_ACCEL; s.milkL=Math.min(CAP,Math.round(((Number(s.milkL)||0)+addL*.98)*1000)/1000); s.milkR=Math.min(CAP,Math.round(((Number(s.milkR)||0)+addR*1.02)*1000)/1000);
    rising=s.letdownPhaseL==='rising'||s.letdownPhaseR==='rising'; if(rising&&flow>=.005*28.3495){ if(!s.letdownStableSince) s.letdownStableSince=now; if(now-s.letdownStableSince>=SWITCH_CONFIRM_MS){ if(s.letdownPhaseL==='rising')s.letdownPhaseL='active';if(s.letdownPhaseR==='rising')s.letdownPhaseR='active';s.letdownPhase='active'; if(s.selectedProgram){ s.mode='expression'; s.rhythmIndex=Math.max(1,Number(s.rhythmIndex)||1); notify(s,'auto','Let-down detected. Moving into the Expression phase.',false); } else if(s.auto){ s.mode='expression'; notify(s,'auto','Let-down detected. Switched to Expression mode.',false); } else if(!s.letdownSuggestionShown){ s.letdownSuggestionShown=true; notify(s,'suggestion','Let-down detected. You can switch to Expression when you are ready.',false); } } }
  ...
  function trigger(id){ var s=st(), match, side, suffix; if(!s) return false; s.lastIgnoredTrigger=''; if(id==='fit-ok') return fitOk(); if(!isActive(s)) return ignored(s,id); match=/^letdown-(start|end)-([lr])$/.exec(id); if(match){side=match[2];suffix=side==='r'?'R':'L';s['letdownPhase'+suffix]=match[1]==='start'?'rising':'falling';s['letdownEventAt'+suffix]=Date.now();s.letdownPhase=match[1]==='start'?'rising':s.letdownPhase;s.letdownStableSince=null;s.noMilkSince=null;s.manualEndSuggestionShown=false;recordLetdown(s,side,match[1]);paint();return true;} if(id==='air-leak'){ s.paused=true; s.severeLeak=true; s.leakSide='r'; s.leakAdjusting=true; s.flowRate=0; s.flowKind='paused'; s.controlNotice={kind:'leak',phase:'warning',text:'Air leak detected',id:Date.now()}; paint(); return true; } if(id==='low-battery'){ s.batteryL=12; s.batteryR=10; notify(s,'low-battery','Battery is running low. You can finish this session, then charge Air 2 soon.',false); paint(); return true; } if(id==='critical-battery') return criticalBattery(); if(id==='minor-leak'){ s.microLeakDuringSession=true; return true; } return false; }
  ...
  function boot(){ install(); wrapLogged(); wrapView(); mount(); window.Air2DemoTriggers={version:58,trigger:trigger,list:function(){return triggers;},sync:sync}; }
