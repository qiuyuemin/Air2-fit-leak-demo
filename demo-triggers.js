(function () {
  ...
  function patchNotice(){ var s=st(), n=s&&s.controlNotice, box=document.querySelector('#demo .c36-notice'), screen=document.querySelector('#demo .v4-control'), p=document.querySelector('#demo .c36-critical-battery p')||document.querySelector('#demo .c36-notice p'); if(n&&p) p.textContent=n.text||''; if(screen){ screen.classList.toggle('has-c36-notice',!!n); screen.classList.toggle('air2-warm-notice',!!(n&&n.backdrop)); screen.classList.toggle('air2-notice-leaving',!!(n&&n.leaving)); } if(box) box.classList.remove('air2-warm-copy'); }
  ...
  function notify(s,k,t,persist,backdrop){ var warm=!!backdrop||k==='auto'||k==='ending'||k==='suggestion'||k==='critical-battery'||String(t||'').length>44,id=Date.now(); s.controlNotice={kind:k,text:t,id:id,startedAt:id,persistent:!!persist,backdrop:warm,leaving:false}; if(!persist){ setTimeout(function(){if(!(s.controlNotice&&s.controlNotice.id===id&&s.controlNotice.kind===k))return;s.controlNotice.leaving=true;s.controlNotice.leavingAt=Date.now();paint();setTimeout(function(){if(s.controlNotice&&s.controlNotice.id===id){s.controlNotice=null;paint()}},1250)},5000); } }
