(function () {
  ...
  var triggers = [
    ...
    ['letdown-start-both','Both let-downs start',true,'letdown'],
    ['letdown-end-both','Both let-downs end',false,'letdown'],
  ...
  function beginSession(s){ s.page='control'; s.modal=null; s.running=true; s.paused=false; s.timer=0; s.milkL=0; s.milkR=0; s.mode='stimulation'; s.letdownPhase='baseline'; s.letdownPhaseL='baseline'; s.letdownPhaseR='baseline'; s.letdownEventAtL=null; s.letdownEventAtR=null; s.air2ModeLetdownConfirmed=false; s.flowRate=BASE_FLOW; s.flowRateL=BASE_FLOW; s.flowRateR=BASE_FLOW; s.flowKind='low'; s.flowKindL='low'; s.flowKindR='low'; s.air2LetdownEvents=[]; s.air2LastPhysicsAt=Date.now(); s.air2SessionEnded=false; s.controlNotice=null; if(window.air2DemoRun) window.air2DemoRun={prompted:false,autoMoved:false,finishing:false,lastMilkTick:0}; }
  ...
  function physics(){ var s=st(), now=Date.now(), flowL, flowR, flow, elapsed, addL, addR, anyActive, activeName, bothEnding; if(!s||!s.running||s.paused) return; flowL=flowNow(s,'l'); flowR=flowNow(s,'r'); flow=Math.max(flowL,flowR); s.flowRateL=Math.round(flowL*1000)/1000; s.flowRateR=Math.round(flowR*1000)/1000; s.flowRate=Math.round(((flowL+flowR)/2)*1000)/1000; s.flowKindL=flowKind(flowL); s.flowKindR=flowKind(flowR); s.flowKind=flowKind(flow); elapsed=Math.max(.25,Math.min(2,(now-(s.air2LastPhysicsAt||now))/1000)); s.air2LastPhysicsAt=now; addL=(flowL/28.3495)*elapsed*DEMO_MILK_ACCEL; addR=(flowR/28.3495)*elapsed*DEMO_MILK_ACCEL; s.milkL=Math.min(CAP,Math.round(((Number(s.milkL)||0)+addL*.98)*1000)/1000); s.milkR=Math.min(CAP,Math.round(((Number(s.milkR)||0)+addR*1.02)*1000)/1000);
